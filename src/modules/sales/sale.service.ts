...
import type { CreateSaleInput, SaleQueryInput, SalesReturnInput } from './sale.validation.js';
...
export async function getSales(query: SaleQueryInput): Promise<PaginatedResult<any>> {
  const result = await saleRepo.findManyPaginated(query);
...
export async function createSale(input: CreateSaleInput) {
  ...
  await recordActivity({
    ...
    details: { saleId: result.id, grandTotal: result.grandTotal, status: result.status },
  });
...
export async function getSalesReturns(query?: { page?: number; limit?: number }) {
  const page = query?.page || 1;
  const limit = query?.limit || 20;
  const result = await saleRepo.findReturnsPaginated(page, limit);
  return buildPaginatedResult(result.data, result.total, result.page, result.limit);
}

export async function getSalesReturnById(id: string) {
  const salesReturn = await saleRepo.findReturnById(id);
  if (!salesReturn) {
...
export async function createSalesReturn(input: SalesReturnInput) {
  ...
  await recordActivity({
    action: 'RETURN',
    module: 'SALES',
    reference: `Return for Sale ${input.saleId}`,
    details: { salesReturnId: result.id, total: result.total, reason: input.reason },
  });
