...
import type { CreatePurchaseInput, PurchaseQueryInput, PurchaseReturnInput } from './purchase.validation.js';
...
export async function getPurchases(query: PurchaseQueryInput): Promise<PaginatedResult<any>> {
  const result = await purchaseRepo.findManyPaginated(query);
...
export async function createPurchase(input: CreatePurchaseInput) {
  ...
  await recordActivity({
    action: 'CREATE',
    module: 'PURCHASES',
    reference: result.purchaseNumber,
    details: { purchaseId: result.id, grandTotal: result.grandTotal, status: result.status },
  });
...
export async function getPurchaseReturns(query?: { page?: number; limit?: number }) {
  ...
  const result = await purchaseRepo.findReturnsPaginated(page, limit);
  return buildPaginatedResult(result.data, result.total, result.page, result.limit);
}

export async function getPurchaseReturnById(id: string) {
  const purchaseReturn = await purchaseRepo.findReturnById(id);
  if (!purchaseReturn) {
...
export async function createPurchaseReturn(input: PurchaseReturnInput) {
  ...
  await recordActivity({
    action: 'RETURN',
    module: 'PURCHASES',
    reference: `Return for Purchase ${input.purchaseId}`,
    details: { purchaseReturnId: result.id, total: result.total, reason: input.reason },
  });
